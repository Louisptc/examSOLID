...
export default class BookService {
    ...
    public async getAllBooks(): Promise<Book[]> {
        ...
        return res.rows.map((row : IBook) => new Book(row.id, row.title, row.author, new Date(row.publishedDate)));
    }
    ...
    public async getBookById(bookId: number): Promise<Book> {
        ...
        if (res.rows.length > 0) {
            ...
            return new Book(row.id, row.title, row.author, new Date(row.published_date));
        }
    ...
    public async createBook(book: Book): Promise<Book> {
        const query = 'INSERT INTO books (title, author, published_date) VALUES ($1, $2, $3) RETURNING *';
    ...
    public async updateBook(book: Book): Promise<Book> {
        const query = 'UPDATE books SET title = $1, author = $2, published_date = $3 WHERE id = $4 RETURNING *';
